...
namespace KineCTRL.DollarRecognizer
{
    ...
    class PointCloudRecognizer
    {
        ...
        /// Returns the class of the closest neighbor in the training set and the distance from the sample.
        /// </summary>
        /// <param name="candidate"></param>
        /// <param name="trainingSet"></param>
        /// <returns></returns>
        public static Tuple<Gesture, float> Classify(Gesture candidate, List<Gesture> trainingSet)
        {
            float minDistance = float.MaxValue;
            Gesture gestureClass = new Gesture();

            foreach (Gesture template in trainingSet)
            {
                float dist = GreedyCloudMatch(candidate.NormalizedGesture.Points, template.NormalizedGesture.Points);
                if (dist < minDistance)
                {
                    minDistance = dist;
            ...
            return new Tuple<Gesture, float>(gestureClass, minDistance);
        }
